...
- A single unit ordered to move was given a formation. The wide corridor a group gets, so it can travel abreast and take the wider way round, was being computed for selections of one, at twice the cost of that unit's own route and to no purpose. One unit now just drives.
- Those three together, over four seven-minute four-way battles: frames that missed a sixtieth of a second fell from 15 to 6, frames over a thirtieth from 3 to 1, and the slowest one frame in a thousand from 13.3 milliseconds to 10.5. The average is unchanged, which is the point - none of this was traded for anything.
...
- Overtaking was built after that and taken out again. A unit held up by traffic going its own way would steer a tank width or two sideways and drive in that lane, which is what a person does in a queue and looks exactly right when you watch it happen. It got argued with the same way as everything else: forty matches on maps its numbers had never been fitted to, against the same build without it. Five of the forty were thrown out because two batches had been run at once and fought over the same generated map, and the first reading of the results was mostly made of matches that never started. Of the thirty-five left, nineteen came out identical, ten of the rest got better and six got worse, and one single match carried five sixths of the average improvement. The time units spend properly stuck went the wrong way, which is the one column the thing had been rebuilt to protect. EA wrote a version of this in 2003 and switched theirs off too.
...
- A second pass with the same tool named three more, and the log did the work each time rather than anyone guessing: the computer player's search for somewhere to build, the angry mob's health count, and a group order for one unit. A fourth is named and not fixed - a computer team sent across the map on its approach path spends 40 milliseconds finding the wide corridor for it, once or twice a match, and that is one search rather than a mistake repeated. It is the next one.
- Two switches that were written into the code years ago, described in a comment and then never connected to anything, now work: one turns the shared group corridor off, the other turns off pricing a route by when other units will be standing in it. Both matter because a change to movement can only be argued with a before and after in the same build - two separate builds differ in more than the thing being measured. The first thing they measured: turning the corridor off costs 13% more time with units standing behind each other, and saves nine tenths of the stutters.
