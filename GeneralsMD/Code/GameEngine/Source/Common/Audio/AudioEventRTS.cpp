...
void AudioEventRTS::generateFilename( void )
{
	...
	if (m_eventInfo->m_soundType == AT_Music || m_eventInfo->m_soundType == AT_Streaming) {
		...
		{ 
			if (m_isLogicalAudio) 
			{
				// Read the shared stream without advancing it: this machine may have sound switched
				// off in the options and never get here at all, and the simulation cannot be allowed
				// to depend on that.
